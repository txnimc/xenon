...
import me.jellysquid.mods.sodium.client.gui.reesesoptions.util.StringUtils;
...
public class SearchTextFieldComponent extends AbstractWidget {
    ...
    public void render(GuiGraphics context, int mouseX, int mouseY, float delta) {
        ...
        if (!string.isEmpty()) {
            ...
            n = context.drawString(this.textRenderer, this.renderTextProvider.apply(string2, this.firstCharacterIndex), n, m, 0xFFFFFFFF);
        }
        ...
        if (!string.isEmpty() && bl && j < string.length()) {
            context.drawString(this.textRenderer, this.renderTextProvider.apply(string.substring(j), this.selectionStart), n, m, 0xFFFFFFFF);
        }
        // Cursor
        if (this.isFocused()) {
