package toni.xenon.extras.darkness;
...
public class DarknessPlus {
    ...
    private static boolean isDark(Level world) {
		if (ExtrasConfig.darknessMode.get() == ExtrasConfig.DarknessMode.OFF) return false;
		...
		} else if (ExtrasTools.isWhitelisted(dimType.location(), ExtrasConfig.darknessDimensionWhiteList)) {
            return true;
	...
	private static float skyFactor(Level world) {
        if (ExtrasConfig.darknessBlockLightOnlyCache || !isDark(world)) return 1;
		...
		final float moon = ExtrasConfig.darknessAffectedByMoonPhaseCache ? world.getMoonBrightness() : 0;
		final float moonInterpolated = (float) Mth.lerp(moon, ExtrasConfig.darknessNewMoonBrightCache, ExtrasConfig.darknessFullMoonBrightCache);
	...
	public static void updateLuminance(float tickDelta, Minecraft client, GameRenderer gameRenderer, float prevFlicker) {
        ...
        for (int skyIndex = 0; skyIndex < 16; ++skyIndex) {
            ...
            var value = ExtrasConfig.darknessMode.get().value;
            ...
            for (int blockIndex = 0; blockIndex < 16; ++blockIndex) {
                ...
                min = Math.max(0.03f * f, ExtrasConfig.darknessMode.get().value);
                red = red * (0.99F - min) + min;
                green = green * (0.99F - min) + min;
                blue = blue * (0.99F - min) + min;
